...
pub struct HealthCheckRegistry {
    checks: HashMap<&'static str, mpsc::Sender<HealthCheckRequest>>,
}
...
impl HealthCheckRegistry {
    ...
    pub fn register(&mut self, name: &'static str) -> mpsc::Receiver<HealthCheckRequest> {
        let (tx, rx) = mpsc::channel(1);
        self.checks.insert(name, tx);
        rx
    }
    ...
    pub async fn all_healthy(&self, timeout: Duration) -> Result<(), &'static str> {
        let results = join_all(self.checks.iter().map(|(name, tx)| async move {
